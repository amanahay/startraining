...
app.get('/api/admin/:table', auth, (req, res, next) => {
  ...
  const order = ['leads', 'program_orders'].includes(table) ? 'created_at DESC, id DESC' :
    ['about_pages', 'service_pages', 'program_pages'].includes(table) ? 'id ASC' : 'sort_order ASC, id DESC';
...
app.post('/api/admin/:table', auth, (req, res, next) => {
  ...
  if (!allowed || table === 'leads' || table === 'program_orders' || table === 'about_pages' || table === 'service_pages' || table === 'program_pages') return next();
...
app.delete('/api/admin/:table/:id', auth, (req, res, next) => {
  const table = req.params.table;
  if (!tableDefinitions[table] || table === 'leads' || table === 'program_orders' || table === 'about_pages' || table === 'service_pages' || table === 'program_pages') return next();
...
app.delete('/api/admin/media/:id', auth, (req, res) => {
  ...
  for (const url of [media.source_url, media.optimized_url].filter(Boolean)) {
    const resolved = path.resolve(root, `.${url}`);
    if (resolved.startsWith(path.resolve(uploadDir)) && fs.existsSync(resolved)) {
      fs.rmSync(resolved, { force: true });
    }
  }
  run('DELETE FROM media_files WHERE id=?', [req.params.id]);
